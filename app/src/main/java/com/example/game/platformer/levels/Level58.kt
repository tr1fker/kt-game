package com.example.game.platformer.levels

import com.example.game.platformer.Level
import com.example.game.platformer.PressurePlateDef
import com.example.game.platformer.RectObj
import com.example.game.platformer.TimedBridgeSegmentDef

/** Одна плита открывает мост через среднюю яму. */
fun level58(): Level = Level(
    startX = 30f,
    startY = -60f,
    platforms = listOf(
        RectObj(0f, 0f, 150f, 20f),
        RectObj(790f, 0f, 210f, 20f),
    ),
    hazards = emptyList(),
    checkpoints = emptyList(),
    goal = RectObj(920f, -70f, 40f, 70f),
    pressurePlates = listOf(
        PressurePlateDef(
            rect = RectObj(85f, -10f, 50f, 10f),
            bridgeGroupId = 0,
            openDurationSec = 6.8f
        )
    ),
    timedBridgeSegments = listOf(
        TimedBridgeSegmentDef(
            groupId = 0,
            rect = RectObj(150f, 0f, 640f, 20f)
        )
    ),
    coinPickups = listOf(
        RectObj(440f, -42f, 22f, 22f),
    )
)
